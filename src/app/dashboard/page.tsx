...
export default function DashboardPage() {
  ...
  const account = accounts.find((item) => item.id === selected?.accountId) ?? {
    bank: "Cuenta no disponible", holder: "Contacta con soporte", accountNumber: "—",
    accountType: "—", achRoutingNumber: "—", wireRoutingNumber: "—", instructions: "",
  };
        ...
        <div className="mx-auto max-w-[1500px] space-y-5">
          ...
          <section className="premium-card overflow-hidden rounded-[1.6rem] p-4 sm:p-5">
            <div className="relative z-10 grid items-center gap-3 md:grid-cols-[180px_1fr_180px]">
              <div className="rounded-2xl border border-[#071A2D]/8 bg-white p-4 shadow-[0_12px_26px_rgba(7,26,45,.07)]"><p className="text-11px text-[#071A2D]/42">Dinero enviado</p><div className="mt-2 flex items-center justify-between"><p className="text-xl font-semibold">{selected ? formatUsd(selected.usdAmount) : "—"}</p><span className="flex items-center gap-2 text-11px font-semibold"><FlagMark country="US"/>USD</span></div></div>
              <div className="relative min-h-[95px]"><FlowCircuit id="dashboard-circuit" className="absolute inset-0 h-full w-full"/><span className="absolute left-1/2 top-1 z-10 -translate-x-1/2 rounded-full border border-[#071A2D]/8 bg-white px-3 py-1 text-10px font-semibold shadow-sm">{selected ? STABLE_STATUS[selected.status].label : "Sin operaciones activas"}</span></div>
              <div className="rounded-2xl bg-[#071A2D] p-4 text-white shadow-[0_16px_32px_rgba(7,26,45,.18)]"><p className="text-11px text-white/42">{selected?.bankReceivedAmount == null ? "Entrega estimada" : "Entrega final conciliada"}</p><div className="mt-2 flex items-center justify-between"><p className="text-xl font-semibold">{selected ? selected.deliveryAmount.toLocaleString() : "—"}</p><div className="flex -space-x-1"><AssetMark asset="USDT" className="h-7 w-7 ring-2 ring-[#071A2D]"/><AssetMark asset="ETH" className="h-7 w-7 ring-2 ring-[#071A2D]"/></div></div></div>
            </div>
          </section>
          ...
          <section className="grid gap-4 2xl:grid-cols-[285px_minmax(0,1fr)_260px]">
            <div className="pathline-surface overflow-hidden rounded-2xl">
              {(filter === "all" || filter === "stable") && operations.map(operation => <button key={operation.id} onClick={() => setSelectedId(operation.id)} className={`w-full border-b border-[#071A2D]/7 p-4 text-left transition-colors ${selected?.id === operation.id ? "bg-[#4DE2B5]/10 shadow-[inset_4px_0_0_#0aa883]" : "hover:bg-[#071A2D]/[.025]"}`}><div className="flex items-start gap-3"><AssetMark asset={operation.asset} className="h-10 w-10"/><div className="min-w-0"><span className="text-11px font-semibold uppercase tracking-[.13em] text-[#087f62]">Stable</span><p className="font-semibold">{operation.reference}</p><p className="mt-1 text-xs font-medium">{formatUsd(operation.usdAmount)} → {operation.deliveryAmount.toLocaleString()} {operation.asset}</p><p className="mt-1 truncate text-xs text-[#071A2D]/42">{operation.senderLegalName ?? "Remitente histórico no registrado"}</p><div className="mt-2"><StatusBadge status={operation.status} /></div></div></div></button>)}
              {(filter === "all" || filter === "remittance") && transfers.map((transfer) => <Link key={transfer.id} href={`/dashboard/history?search=${encodeURIComponent(transfer.reference)}`} className="block w-full border-b border-[#071A2D]/7 p-4 text-left hover:bg-[#071A2D]/[.025]"><div className="flex gap-3"><div className="grid h-10 w-10 place-items-center rounded-full bg-[#4C7DFF]/10 text-[#356de5]"><Send className="h-5 w-5" /></div><div className="min-w-0"><span className="text-11px font-semibold uppercase tracking-[.13em] text-[#356de5]">Remesa</span><p className="font-semibold">{transfer.reference}</p><p className="mt-1 text-xs font-medium">{transfer.send_amount.toLocaleString()} {transfer.send_currency} ? {transfer.receive_amount.toLocaleString()} {transfer.receive_currency}</p><div className="mt-2"><span className={`status-pill status-${transfer.status === "completed" ? "done" : transfer.status === "processing" ? "processing" : transfer.status === "failed" ? "blocked" : "waiting"}`}>{transfer.status === "completed" ? "Completada" : transfer.status === "processing" ? "Procesando" : transfer.status === "failed" ? "Fallida" : "Pendiente"}</span></div></div></div></Link>)}
              {((filter === "stable" && operations.length === 0) || (filter === "remittance" && transfers.length === 0) || (filter === "all" && operations.length === 0 && transfers.length === 0)) && <div className="p-6 text-center text-xs text-[#071A2D]/45">No hay operaciones para este filtro.</div>}
            </div>

            {selected && filter !== "remittance" && <div className="pathline-surface min-w-0 rounded-2xl p-4 sm:p-6">
              <div className="flex flex-col justify-between gap-3 border-b border-[#071A2D]/8 pb-5 sm:flex-row sm:items-start"><div className="flex items-center gap-3"><AssetMark asset={selected.asset} className="h-11 w-11"/><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#087f62]">Patzi Stable</p><h3 className="mt-1 text-2xl font-semibold">{selected.reference}</h3><p className="mt-1 text-sm font-medium">{formatUsd(selected.usdAmount)} → {selected.deliveryAmount.toLocaleString()} {selected.asset}</p></div></div><StatusBadge status={selected.status} /></div>
              <div className="mt-6 grid grid-cols-3 gap-2 sm:grid-cols-6">{timeline.map((step,index) => { const done = index <= currentStep && currentStep >= 0; const active = index === currentStep; return <div key={step.status} className="relative text-center"><div className={`mx-auto grid h-7 w-7 place-items-center rounded-full border-2 text-xs font-black ${active ? "border-[#071A2D] bg-[#4DE2B5]" : done ? "border-[#0aa883] bg-[#0aa883] text-white" : "border-[#071A2D]/10 bg-white text-[#071A2D]/25"}`}>{done && !active ? <Check className="h-3.5 w-3.5" /> : index+1}</div><p className={`mt-2 text-xs font-bold ${active ? "text-[#356de5]" : done ? "text-[#071A2D]" : "text-[#071A2D]/30"}`}>{step.label}</p></div>; })}</div>
              ...
              <div className="mt-5 grid gap-3 lg:grid-cols-3">
                ...
                <div className="rounded-2xl border border-[#071A2D]/9 p-4"><div className="mb-4 flex items-center gap-2 text-xs font-black"><FileText className="h-4 w-4" />Comprobante de pago</div>{!selected.senderId ? <Link href={`/dashboard/stable?operation=${selected.id}`} className="flex flex-col items-center rounded-xl border border-[#FF765B]/25 bg-[#FF765B]/10 p-5 text-center text-[#A13E2C]"><AlertTriangle className="h-6 w-6" /><span className="mt-2 text-xs font-black">Completar datos del remitente</span><span className="mt-1 text-xs opacity-75">Obligatorio antes de subir el PDF</span></Link> : selected.proof ? <><div className="flex items-center gap-3 rounded-xl bg-[#F5F7F2] p-3"><FileText className="h-7 w-7 text-[#d95943]" /><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{selected.proof.name}</p><p className="text-xs text-[#071A2D]/40">{Math.round(selected.proof.size/1024)} KB · PDF</p></div><button onClick={handleDownload} aria-label="Descargar PDF"><Download className="h-4 w-4" /></button></div>{selected.status === "correction_requested" && <label className="mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-[#071A2D]/10 py-2 text-xs font-bold"><Upload className="h-3.5 w-3.5" />Reemplazar PDF<input type="file" accept="application/pdf" className="hidden" onChange={handleProof} /></label>}</> : <label className="flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-[#071A2D]/12 p-5 text-center"><Upload className="h-6 w-6 text-[#087f62]" /><span className="mt-2 text-xs font-black">Subir comprobante PDF</span><span className="mt-1 text-xs text-[#071A2D]/40">Máximo 5 MB</span><input type="file" accept="application/pdf" className="hidden" onChange={handleProof} /></label>}</div>
                <div className="rounded-2xl border border-[#071A2D]/9 p-4"><div className="mb-4 flex items-center justify-between text-xs font-semibold"><span className="flex items-center gap-2"><WalletCards className="h-4 w-4" />Wallet de destino</span><AssetChip asset="ETH"/></div><p className="break-all text-sm font-semibold">{shortWallet(selected.walletAddress)}</p><p className="mt-1 text-xs text-[#071A2D]/45">{selected.network}</p><div className="mt-4 flex gap-2"><button onClick={() => copy(selected.walletAddress,"Wallet")} className="grid h-9 w-9 place-items-center rounded-lg bg-[#F5F7F2]"><Copy className="h-4 w-4" /></button><a href={`https://etherscan.io/address/${selected.walletAddress}`} target="_blank" rel="noreferrer" className="grid h-9 w-9 place-items-center rounded-lg bg-[#F5F7F2]" aria-label="Ver wallet en Etherscan"><ExternalLink className="h-4 w-4" /></a></div><p className="mt-4 rounded-xl bg-[#FF765B]/10 p-3 text-xs leading-4 text-[#a13e2c]">La wallet y red no pueden cambiarse después de verificar el pago.</p></div>
              </div>
              <div className="mt-5"><div className="mb-3"><p className="text-sm font-semibold">Documentos de la operación</p><p className="mt-1 text-xs text-[#071A2D]/42">Comprobante, factura y contrato privados</p></div><OperationDocuments operation={selected} /></div>
              <div className="mt-5 rounded-2xl bg-[#F5F7F2] p-4"><div className={`grid gap-3 ${selected.bankReceivedAmount == null ? "sm:grid-cols-3" : "sm:grid-cols-5"}`}><div><p className="text-xs text-[#071A2D]/40">Enviado</p><p className="mt-1 text-lg font-black">{formatUsd(selected.usdAmount)}</p></div>{selected.bankReceivedAmount != null&&<><div><p className="text-xs text-[#071A2D]/40">Banco recibió</p><p className="mt-1 text-lg font-black text-[#356DE5]">{formatUsd(selected.bankReceivedAmount)}</p></div><div><p className="text-xs text-[#071A2D]/40">Fee bancario</p><p className="mt-1 text-lg font-black text-[#d95943]">{formatUsd(selected.bankFeeAmount ?? 0)}</p></div></>}<div><p className="text-xs text-[#071A2D]/40">Comisión Patzi 10%</p><p className="mt-1 text-lg font-black text-[#d95943]">{formatUsd(selected.feeAmount)}</p></div><div><p className="text-xs text-[#071A2D]/40">Recibirás</p><p className="mt-1 text-lg font-black text-[#087f62]">{selected.deliveryAmount.toLocaleString()} {selected.asset}</p></div></div>{selected.bankReceivedAmount==null&&<p className="mt-3 text-xs text-[#071A2D]/45">El resultado es estimado hasta que Patzi concilie el abono real del banco.</p>}</div>
            </div>}

            {selected && filter !== "remittance" && <aside className="pathline-surface rounded-2xl p-4"><h3 className="flex items-center gap-2 text-sm font-black"><Clipboard className="h-4 w-4" />Actividad reciente</h3><div className="mt-5 space-y-5">{selected?.history.slice().reverse().map((entry,index) => <div key={entry.id} className="relative flex gap-3"><span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${index===0 ? "bg-[#0aa883]" : "bg-[#071A2D]/18"}`} /><div><p className="text-xs font-black">{entry.label}</p><p className="mt-1 text-xs text-[#071A2D]/40">{new Date(entry.createdAt).toLocaleString("es-ES",{dateStyle:"short",timeStyle:"short"})}</p></div></div>)}</div>{selected?.txHash && <div className="mt-6 rounded-xl border border-[#071A2D]/10 p-3"><p className="text-xs font-bold text-[#071A2D]/40">Hash de transacción</p><button onClick={() => copy(selected.txHash!,"Hash")} className="mt-2 flex w-full items-center gap-2 text-left text-xs font-black"><span className="truncate">{selected.txHash}</span><Copy className="h-3 w-3 shrink-0" /></button></div>}</aside>}
          </section>
        </div>
      </div>
    </>
