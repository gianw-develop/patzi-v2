...
const operations = [
  ...
  { ref: "PTZ-80398", type: "Stable", amount: "$2,000", output: "1,800 USDC", status: "Preparando", tone: "preparing", asset: "USDC" as const },
...
function HeroProductPreview() {
  const { t } = useLanguage();
  return (
    <div className="premium-stage relative min-h-[520px] overflow-hidden rounded-[2.5rem] border border-[#071A2D]/10 p-5 shadow-[0_45px_100px_rgba(7,26,45,.18)] sm:p-8">
      ...
      <div className="depth-window relative z-10 mt-6 overflow-hidden rounded-[1.65rem] border border-white/80 bg-white/92 shadow-[0_40px_80px_rgba(7,26,45,.2),0_1px_0_white_inset] backdrop-blur-xl sm:ml-7">
        ...
        <div className="grid sm:grid-cols-[1.02fr_.98fr]">
          ...
          <div className="relative overflow-hidden bg-[#071A2D] p-5 text-white">
            ...
            <div className="mt-4 flex items-center justify-between"><AssetChip asset="ETH" dark/><div className="flex -space-x-1"><AssetMark asset="USDT" className="h-6 w-6 ring-2 ring-[#071A2D]"/><AssetMark asset="USDC" className="h-6 w-6 ring-2 ring-[#071A2D]"/></div></div>
...
export default function Home() {
  const { t } = useLanguage();
  return (
    <main className="premium-shell overflow-hidden bg-[#FBFCF9] text-[#071A2D]">
      ...
      <section className="relative bg-white"><div className="mx-auto grid min-h-[650px] max-w-[1240px] items-center gap-12 px-5 py-14 lg:grid-cols-[.78fr_1.22fr] lg:px-8"><div className="relative z-10"><div className="inline-flex items-center gap-2 rounded-full border border-[#071A2D]/8 bg-[#F3F8F5] px-3 py-2 text-[10px] font-semibold text-[#087F62]"><span className="h-2 w-2 rounded-full bg-[#4DE2B5] shadow-[0_0_0_5px_rgba(77,226,181,.15)]"/>{t("Remesas + Stablecoins, una sola cuenta")}</div><h1 className="mt-6 max-w-[540px] text-[3.15rem] font-semibold leading-[.98] tracking-[-.06em] sm:text-[4.15rem]">{t("Tu dinero, con una ruta más clara.")}</h1><p className="mt-6 max-w-[500px] text-[1.05rem] leading-7 text-[#071A2D]/60">{t("Envía remesas con confianza. Y, si tu cuenta está habilitada, cambia USD a USDT o USDC sin salir de Patzi.")}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/dashboard/send" className="inline-flex h-12 items-center justify-center gap-3 rounded-xl bg-[#071A2D] px-6 text-xs font-semibold text-white shadow-[0_16px_28px_rgba(7,26,45,.19)]">{t("Enviar dinero")} <ArrowRight className="h-4 w-4"/></Link><a href="#stable" className="inline-flex h-12 items-center justify-center gap-3 rounded-xl border border-[#071A2D]/14 bg-white px-6 text-xs font-semibold shadow-sm">{t("Conocer Patzi Stable")} <ArrowRight className="h-4 w-4"/></a></div><div className="mt-8 flex flex-wrap gap-4 text-[10px] text-[#071A2D]/48"><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#087F62]"/>{t("Cuenta verificada")}</span><span className="flex items-center gap-2"><FileCheck2 className="h-4 w-4 text-[#2775CA]"/>{t("Comprobantes PDF")}</span><span className="flex items-center gap-2"><Network className="h-4 w-4 text-[#627EEA]"/>Ethereum ERC-20</span></div></div><HeroProductPreview/></div></section>
