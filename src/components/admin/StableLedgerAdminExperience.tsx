...
function Status({ operation }: { operation: LedgerOperation }) {
  const { t } = useLanguage();
  if (operation.adminNote === HISTORICAL_IMPORT_NOTE) return <span className="rounded-full bg-violet-50 px-3 py-1.5 text-[10px] font-semibold text-violet-700">{t("Registro histórico")}</span>;
  if (["payment_received","preparing","completed"].includes(operation.status)) return <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1.5 text-[10px] font-semibold text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5"/>{t("Aprobado")}</span>;
  if (operation.status === "blocked") return <span className="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-3 py-1.5 text-[10px] font-semibold text-red-600"><XCircle className="h-3.5 w-3.5"/>{t("No aprobado")}</span>;
  if (operation.status === "waiting_payment") return <span className="rounded-full bg-amber-50 px-3 py-1.5 text-[10px] font-semibold text-amber-700">{t("Falta comprobante")}</span>;
...
export default function StableLedgerAdminExperience() {
  ...
  const [documentsOperation, setDocumentsOperation] = useState<LedgerOperation | null>(null);
  ...
  const showProof = async (proof: LedgerProof, payout=false) => {
    ...
    catch(error){toast.error(error instanceof Error?error.message:t("No se pudo abrir el comprobante."));}
  };

  const approve = async (event:FormEvent) => {
    event.preventDefault(); if(!review||!validActual)return;
    setBusy("approve");
    try{await approveDeposit(review.id,actual);toast.success(language === "en" ? `${review.reference} approved and ${formatUsdt(generated)} credited` : `${review.reference} aprobado y ${formatUsdt(generated)} acreditados`);setReview(null);}
    catch(error){toast.error(error instanceof Error?error.message:t("No se pudo aprobar."));}
    finally{setBusy("");}
  };

  const reject = async (event:FormEvent) => {
    event.preventDefault();if(!rejecting)return;setBusy("reject");
    try{await rejectDeposit(rejecting.id,rejectReason);toast.success(t("Depósito marcado como no aprobado"));setRejecting(null);setRejectReason("");}
    catch(error){toast.error(error instanceof Error?error.message:t("No se pudo rechazar."));}
    finally{setBusy("");}
  };
  ...
  const submitPayout = async (event:FormEvent) => {
    event.preventDefault();if(!selectedClient||!payoutFile)return toast.error(t("Carga el comprobante del pago."));
    setBusy("payout");
    try{await recordPayout(selectedClient.id,Number(payoutAmount),payoutFile,payoutRequestId||undefined);toast.success(t("Abono USDT registrado"));setPayoutOpen(false);setPayoutAmount("");setPayoutFile(null);setPayoutRequestId("");}
    catch(error){toast.error(error instanceof Error?error.message:t("No se pudo registrar el abono."));}
    finally{setBusy("");}
  };

  if(loading)return <><Header title={t("Pagos Stable")} subtitle={t("Cargando control USDT")}/><div className="grid flex-1 place-items-center bg-[#F5F7F2]"><LoaderCircle className="h-8 w-8 animate-spin text-[#0AA883]"/></div></>;

  return <>
    <Header title={t("Pagos Stable")} subtitle={t("Conciliación USD, crédito USDT y abonos parciales")}/>
    <main className="pathline-grid flex-1 bg-[#F5F7F2] p-4 sm:p-6 xl:p-8"><div className="mx-auto max-w-[1560px] space-y-5">
      ...
      <section className="rounded-[1.6rem] border border-[#071A2D]/7 bg-white p-5 shadow-[0_18px_45px_rgba(7,26,45,.05)]"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="premium-kicker text-[#087F62]">{t("Tesorería USDT")}</p><h2 className="mt-1 text-xl font-semibold">{t("Abonos y saldos de clientes")}</h2></div><Button onClick={()=>openPayout()} disabled={clients.length===0} className="h-11 bg-[#071A2D] text-white">{t("Registrar abono USDT")}</Button></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-[10px] uppercase tracking-[.12em] text-[#071A2D]/42"><tr>{["Cliente","Wallet USDT","Saldo disponible","Solicitud pendiente","Acción"].map((h)=><th key={h} className="px-3 py-2 font-semibold">{t(h)}</th>)}</tr></thead><tbody>{clients.map((client)=>{const wallet=wallets.find((item)=>item.userId===client.id);const balance=balanceFor(operations,payouts,client.id);const request=payoutRequests.find((item)=>item.userId===client.id&&item.status==="pending");return <tr key={client.id} className="border-t border-[#071A2D]/6"><td className="px-3 py-3"><b>{client.name}</b><p className="text-xs text-[#071A2D]/42">{client.email}</p></td><td className="px-3 py-3">{wallet?<><span className="font-mono text-xs">{maskWallet(wallet.address)}</span><button onClick={()=>void verifyWallet(client.id,!wallet.verified).then(()=>toast.success(wallet.verified?t("Wallet marcada como pendiente"):t("Wallet verificada"))).catch((e)=>toast.error(e instanceof Error?e.message:t("No se pudo actualizar")))} className={`ml-2 rounded-full px-2 py-1 text-[9px] font-semibold ${wallet.verified?"bg-emerald-50 text-emerald-700":"bg-amber-50 text-amber-700"}`}>{wallet.verified?t("Verificada"):t("Verificar")}</button></>:<span className="text-xs text-[#071A2D]/38">{t("Sin wallet")}</span>}</td><td className="px-3 py-3 font-semibold text-[#087F62]">{formatUsdt(balance.available)}</td><td className="px-3 py-3">{request?<span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700">{formatUsdt(request.amount)}</span>:<span className="text-xs text-[#071A2D]/38">—</span>}</td><td className="px-3 py-3"><Button onClick={()=>openPayout(client.id,request?.id,request?.amount)} disabled={!wallet?.verified||balance.available<=0} variant="outline" className="h-9 text-xs">{t("Abonar")}</Button></td></tr>})}</tbody></table></div>
        <div className="mt-5 border-t border-[#071A2D]/7 pt-4"><h3 className="text-sm font-semibold">{t("Historial de abonos")}</h3><div className="mt-3 space-y-2">{payouts.slice(0,10).map((payout)=>{const client=clients.find((item)=>item.id===payout.userId);return <div key={payout.id} className="grid items-center gap-2 rounded-xl bg-[#F7F9F7] px-4 py-3 text-sm sm:grid-cols-[1fr_.8fr_.8fr_auto]"><span>{client?.name??t("Cliente")}</span><b className="text-[#087F62]">{formatUsdt(payout.amount)}</b><span className="text-xs text-[#071A2D]/45">{new Date(payout.paidAt).toLocaleString(language === "en" ? "en-US" : "es-ES",{dateStyle:"medium",timeStyle:"short"})}</span><button onClick={()=>void showProof(payout.proof,true)} className="text-xs font-semibold">{t("Ver comprobante")}</button></div>})}{payouts.length===0&&<p className="rounded-xl bg-[#F7F9F7] p-4 text-sm text-[#071A2D]/45">{t("Todavía no hay abonos USDT.")}</p>}</div></div>
      </section>
    </div></main>

    {review&&<Modal onClose={()=>setReview(null)}><ModalHeader title={t("Conciliar depósito")} subtitle={`${review.reference} · ${review.senderName}`} onClose={()=>setReview(null)}/><form onSubmit={approve} className="p-5"><div className="rounded-xl bg-[#F7F9F7] p-4"><p className="text-xs text-[#071A2D]/45">{t("Monto declarado por el cliente")}</p><p className="mt-1 text-xl font-semibold">{formatMoney(review.declaredAmount)}</p></div><label className="mt-4 block text-xs font-semibold">{t("Monto realmente recibido")}<div className="relative mt-1.5"><input autoFocus required type="number" min="0.01" max={review.declaredAmount} step="0.01" value={actualReceived} onChange={(e)=>setActualReceived(e.target.value)} className={`${inputClass} pr-12 text-lg font-semibold`}/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold">USD</span></div></label><div className="mt-4 space-y-3 rounded-xl border border-[#071A2D]/7 p-4 text-sm"><div className="flex justify-between"><span className="text-[#071A2D]/48">{t("Fee bancario")}</span><b>{formatMoney(bankFee)}</b></div><div className="flex justify-between"><span className="text-[#071A2D]/48">{t("Comisión Patzi 10%")}</span><b>{formatMoney(patziFee)}</b></div><div className="flex justify-between border-t border-[#071A2D]/7 pt-3 text-[#087F62]"><span className="font-semibold">{t("Saldo USDT generado")}</span><b className="text-xl">{formatUsdt(generated)}</b></div></div>{review.proof&&<button type="button" onClick={()=>void showProof(review.proof!)} className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#071A2D]/9 text-sm font-semibold"><FileText className="h-4 w-4"/>{t("Ver comprobante")}</button>}<p className="mt-3 text-xs text-[#071A2D]/45">{t("La comisión se calcula sobre el monto realmente recibido.")}</p><div className="mt-5 grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={()=>{setRejecting(review);setReview(null);}} className="h-11 border-red-200 text-red-600">{t("No aprobar")}</Button><Button disabled={!validActual||busy==="approve"} className="h-11 bg-[#0AA883] text-white">{busy==="approve"?<LoaderCircle className="mr-2 h-4 w-4 animate-spin"/>:null}{t("Aprobar y acreditar")}</Button></div></form></Modal>}

    {rejecting&&<Modal onClose={()=>setRejecting(null)}><ModalHeader title={t("No aprobar depósito")} subtitle={rejecting.reference} onClose={()=>setRejecting(null)}/><form onSubmit={reject} className="p-5"><label className="text-xs font-semibold">{t("Motivo para el cliente")}<textarea value={rejectReason} onChange={(e)=>setRejectReason(e.target.value)} className="mt-1.5 min-h-28 w-full rounded-xl border border-[#071A2D]/10 bg-white p-3 text-sm outline-none focus:border-red-300" placeholder={t("Explica por qué no fue aprobado.")}/></label><Button disabled={busy==="reject"} className="mt-4 h-11 w-full bg-[#D9563E] text-white">{busy==="reject"?<LoaderCircle className="mr-2 h-4 w-4 animate-spin"/>:null}{t("Confirmar no aprobación")}</Button></form></Modal>}
