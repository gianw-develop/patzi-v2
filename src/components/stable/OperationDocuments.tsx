...
export default function OperationDocuments({
  ...
  const openProof = async () => {
    if (!operation.proof || !(await downloadProof(operation.proof))) {
  ...
  const openDocument = async (type: StableDocumentType) => {
    ...
    if (!document || !(await downloadStableDocument(document))) {
      toast.error("No se pudo abrir el documento privado.");
    }
  };
    ...
    <div className={cn("grid gap-3 sm:grid-cols-3", className)}>
      <article className="rounded-2xl border border-[#071A2D]/9 bg-white p-3.5 shadow-[0_10px_28px_rgba(7,26,45,.05)]">
        <div className="flex items-start justify-between gap-3">
          ...
          {operation.proof && <button type="button" onClick={() => void openProof()} className="grid h-8 w-8 place-items-center rounded-lg border border-[#071A2D]/9" aria-label="Descargar comprobante"><Download className="h-3.5 w-3.5" /></button>}
        </div>
        ...
        return (
          <article key={type} className="rounded-2xl border border-[#071A2D]/9 bg-white p-3.5 shadow-[0_10px_28px_rgba(7,26,45,.05)]">
            <div className="flex items-start justify-between gap-3">
              ...
              {document && <button type="button" onClick={() => void openDocument(type)} className="grid h-8 w-8 place-items-center rounded-lg border border-[#071A2D]/9" aria-label={`Descargar ${documentMeta[type].label}`}><Download className="h-3.5 w-3.5" /></button>}
            </div>
            ...
            {admin ? (
              <label className="mt-3 flex h-8 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-[#071A2D]/10 bg-[#F6F8F6] text-[9px] font-semibold text-[#071A2D] transition-colors hover:border-[#0AA883]">
