...
export default function StableDashboardOverview() {
  ...
  return <>
    <Header title={`${t("Hola")}, ${full_name?.split(" ")[0] || t("bienvenido")}`} subtitle={t("Tu dinero y tus operaciones, en una sola vista.")} />
      ...
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        <section className="grid gap-4 xl:grid-cols-[1.3fr_.7fr]">
          <div className="overflow-hidden rounded-[1.7rem] bg-[#071A2D] p-6 text-white shadow-[0_24px_55px_rgba(7,26,45,.18)] sm:p-7">
            <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><p className="premium-kicker text-[#4DE2B5]">{t("Saldo disponible")}</p><p className="mt-3 text-4xl font-semibold tracking-[-.04em] sm:text-5xl">{formatUsdt(balance.available)}</p><p className="mt-2 text-sm text-white/46">{t("Listo para solicitar un abono parcial o total.")}</p></div><AssetMark asset="USDT" className="h-14 w-14 ring-4 ring-white/8" /></div>
            <div className="mt-7 grid grid-cols-2 gap-3 border-t border-white/10 pt-5 text-sm"><div><p className="text-xs text-white/40">{t("USDT generado")}</p><b className="mt-1 block">{formatUsdt(balance.credited)}</b></div><div><p className="text-xs text-white/40">{t("USDT pagado")}</p><b className="mt-1 block">{formatUsdt(balance.paid)}</b></div></div>
          </div>
          <div className="rounded-[1.7rem] border border-[#071A2D]/7 bg-white p-6 shadow-[0_18px_45px_rgba(7,26,45,.05)]">
            ...
            <p className="mt-6 truncate font-mono text-base font-semibold">{wallet ? maskWallet(wallet.address) : t("Configúrala una sola vez")}</p><p className="mt-2 text-xs leading-5 text-[#071A2D]/45">{t("Esta dirección permanente se utilizará para todos tus pagos.")}</p><Link href="/dashboard/stable" className="mt-5 inline-flex h-11 w-full items-center justify-center rounded-xl border border-[#071A2D]/10 text-sm font-semibold">{t("Gestionar wallet")}</Link>
          </div>
        </section>

        {stable_eligible && <div className="flex flex-col justify-between gap-3 rounded-2xl border border-[#0AA883]/20 bg-[#E7FAF3] px-5 py-4 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-[#087F62]"/><div><p className="font-semibold">{t("Patzi Stable habilitado")}</p><p className="text-xs text-[#071A2D]/50">{t("Tu cuenta puede registrar depósitos USD para generar saldo USDT.")}</p></div></div><span className="text-xs font-semibold text-[#087F62]">{t("KYC verificado · Cuenta apta")}</span></div>}

        <section className="rounded-[1.7rem] border border-[#071A2D]/7 bg-white shadow-[0_18px_45px_rgba(7,26,45,.05)]">
          <div className="flex items-center justify-between border-b border-[#071A2D]/7 px-5 py-4 sm:px-6"><div><p className="premium-kicker text-[#087F62]">{t("Actividad")}</p><h2 className="mt-1 text-xl font-semibold">{t("Últimos depósitos Stable")}</h2></div><Link href="/dashboard/stable" className="inline-flex items-center gap-2 text-xs font-semibold text-[#087F62]">{t("Ver todos")} <ArrowRight className="h-4 w-4"/></Link></div>
          <div className="divide-y divide-[#071A2D]/6">{operations.slice(0,5).map((operation)=><div key={operation.id} className="grid gap-3 px-5 py-4 text-sm sm:grid-cols-[1fr_.7fr_.8fr_auto] sm:items-center sm:px-6"><div><b>{operation.senderName}</b><p className="mt-1 text-xs text-[#071A2D]/42">{new Date(`${operation.depositDate}T12:00:00`).toLocaleDateString(language === "en" ? "en-US" : "es-ES",{dateStyle:"medium"})}</p></div><b>{formatMoney(operation.declaredAmount)}</b><span className="inline-flex items-center gap-2 text-xs text-[#071A2D]/55"><Clock3 className="h-4 w-4"/>{t(labels[operation.status]??"En revisión")}</span><span className="text-xs font-semibold text-[#087F62]">{operation.bankReceivedAmount==null?t("Pendiente"):formatUsdt(operation.generatedUsdt)}</span></div>)}{operations.length===0&&<p className="px-6 py-8 text-center text-sm text-[#071A2D]/45">{t("Aún no tienes depósitos Stable registrados.")}</p>}</div>
        </section>
        {transfers.length > 0 && <p className="text-center text-xs text-[#071A2D]/42">{language === "en" ? `You also have ${transfers.length} remittance${transfers.length === 1 ? "" : "s"} in your history.` : `También tienes ${transfers.length} remesa${transfers.length === 1 ? "" : "s"} en tu historial.`}</p>}
      </div>
    </main>
  </>;
}
