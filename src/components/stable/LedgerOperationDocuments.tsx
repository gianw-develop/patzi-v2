...
export default function LedgerOperationDocuments({
  ...
  useEffect(() => {
    let active = true;
    void refresh()
      .catch((error) => active && toast.error(error instanceof Error ? error.message : "No se pudieron cargar los documentos."))
  ...
  const open = async (document: StoredDocument) => {
    ...
    if (error || !data?.signedUrl) return toast.error("No se pudo abrir el documento privado.");
  ...
  const download = async (document: StoredDocument) => {
    ...
    if (error || !data?.signedUrl) return toast.error("No se pudo descargar el documento privado.");
  ...
  const upload = async (type: DocumentType, event: ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = "";
    if (!file) return;
    if (file.type !== "application/pdf") return toast.error("La factura o contrato debe ser PDF.");
    if (file.size > 10 * 1024 * 1024) return toast.error("El PDF supera el límite de 10 MB.");
    ...
    try {
      ...
      if (!auth.user) throw new Error("Tu sesión ha caducado.");
      ...
      await refresh();
      toast.success(`${meta[type].label} guardado correctamente.`);
    } catch (error) {
      ...
      {(["invoice", "contract"] as DocumentType[]).map((type) => {
        ...
        return <article key={type} className="rounded-2xl border border-[#071A2D]/8 bg-white p-4 shadow-[0_12px_30px_rgba(7,26,45,.05)]">
          <div className="flex items-start justify-between gap-3">
            ...
            {document && <div className="flex gap-2"><button type="button" onClick={() => void open(document)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[#071A2D]/9 px-3 text-xs font-semibold"><Eye className="h-4 w-4" />Abrir</button><button type="button" onClick={() => void download(document)} className="grid h-9 w-9 place-items-center rounded-lg border border-[#071A2D]/9" aria-label={`Descargar ${meta[type].label}`}><Download className="h-4 w-4" /></button></div>}
          </div>
          <p className="mt-4 text-sm font-semibold">{meta[type].label}</p>
          <p className="mt-1 truncate text-xs text-[#071A2D]/45">{document?.file_name ?? meta[type].hint}</p>
          {document && <span className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-[#E7FAF3] px-2.5 py-1 text-[10px] font-semibold text-[#087F62]"><CheckCircle2 className="h-3 w-3" />Disponible para el cliente</span>}
          {admin ? <label className="mt-4 flex h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border border-[#071A2D]/10 bg-[#F7F9F7] text-xs font-semibold transition hover:border-[#0AA883]">
            {busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}{busy ? "Guardando…" : document ? "Reemplazar PDF" : "Subir PDF"}
            <input type="file" accept="application/pdf" disabled={busy} className="hidden" onChange={(event) => void upload(type, event)} />
          </label> : !document && <span className="mt-4 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500">Pendiente de Patzi</span>}
        </article>;
      })}
    </div>
